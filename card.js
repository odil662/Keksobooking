...
renderMapCard: function(ads) {
  var clonedMapCardTemplate = mapCardTemplate.cloneNode(true);
  ...
  var popupFeatures = clonedMapCardTemplate.querySelector('.popup__features');
  ...
  for (let i = 0; i < ads.offer.features.length; i++) {
    var liTextContent = ads.offer.features[i];
    var liCreate = document.createElement('li');
    liCreate.classList.add('feature');
    liCreate.classList.add(`feature--${liTextContent}`);
    var popupFeaturesChildCreate = popupFeatures.appendChild(liCreate); 
  }}
      
  clonedMapCardTemplate.querySelectorAll('p')[4].textContent = ads.offer.description;
      
  var liMapCardPicturesParent = clonedMapCardTemplate.querySelector('.popup__pictures');
  ...
  for (let i = 0; i < ads.offer.photos.length; i++) {
    var liCreate = document.createElement('li');
    var liMapCardPicturesCreate = liMapCardPicturesParent.appendChild(liCreate);
  }} 

  var mapCardPicturesParent = liMapCardPicturesParent.querySelectorAll('li');

  for (let i = 0; i < mapCardPicturesParent.length; i++) {   
    var imgCreate = document.createElement('img');
    var liChildCreate = mapCardPicturesParent[i].appendChild(imgCreate);
