(function () {
    window.load(function (ads) {
        ...
        let mapPinPostedAd = document.querySelectorAll('.map__pinPostedAd');
        ...
        function filter () {
            ...
            function filterByType () {
                for (let i = 0; i < ads.length; i++) {
                    let ad = ads[i];
                    ad.offer.type === filters.typeFilter ? window.mapPinPostedAd[i].classList.remove('map__pinCurtain') : window.mapPinPostedAd[i].classList.add('map__pinCurtain');
                }
            };
